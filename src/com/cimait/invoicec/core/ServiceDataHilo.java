...
public class ServiceDataHilo extends GenericTransaction{
	...
	public ServiceDataHilo(InfoEmpresa infoEmpresa, Emisor emitir, List listError, List ListWarn,List listErrorEstados,List listWarnEstados){
	...
	public void atiendeHilo() throws Exception
	{
		...
		// if approved or in contingency? stop thread
		if (ls_statusDocumento.equals("AT") || ls_statusDocumento.equals("CT")) {
			...
			mensaje = estado + ",El Documento ya se encuentra en estado "
					+ estado + " por lo cual no se procede a reprocesar. Ruc::"
					+ emite.getInfEmisor().getRuc() + " Tipo de Documento::"
					+ emite.getInfEmisor().getTipoComprobante()
					+ "Nro Documento : " + documentNumber;
			...
			//
			File documentFile = new File(infoEmp.getDirRecibidos() + documentFileName);
			...
			LOGGER.debug("Firmando documento :" + documentNumber);
			Document doc=null;
			
			String resp = X509Utils.firmaValidaArchivo(documentFile , infoEmp.getDirFirmados(), RUC, Util.private_key_alias, infoEmp.getClaveFirma(), infoEmp.getRutaFirma());
			
			File documentSignedFile = new File(infoEmp.getDirFirmados()+documentFileName);
			
			boolean documentAccepted = false;
			if (documentSignedFile.exists() && resp == null) {
				...
				try {
					LOGGER.debug("Enviando archivo a SRI : " + documentNumber);
					 ...
					 //se intenta 10 veces la solicitud de recepcion
					 for (i=0; i<10 ; i++) {
		        	  try{
		        	  		respuestaRecepcion = solicitudRecepcion(documentSignedFile, emite, ListErrorGeneral, ListWarnGeneral);
		        	  		if (respuestaRecepcion != null) break;
		        	  }catch(Exception e){
	        			  efin = e;
		  	  		...
		  	  		if (respuestaRecepcion == null) {
        	  			LOGGER.debug("Error al enviar solicitud de recepcion de " + documentNumber + " sin respuesta de SRI"  );
        	  			respuestaRecepcion = new RespuestaSolicitud();
		        		respuestaRecepcion.setEstado("SIN-RESPUESTA");
		        		//se va por contingencia 
		        		if (efin != null) efin.printStackTrace();
        	  		}
		        	  ...
		        	  if (respuestaRecepcion.getEstado().trim().equals("RECIBIDA")) {
			        		  ...
			        		  if (infoAutorizacion[0].trim().equals("AUTORIZADO")) {
								...
								if ((infoAutorizacion[0].equals("SIN-RESPUESTA"))||(infoAutorizacion[0].equals("SIN-CERTIFICADORA"))){
									
									saveLog(documentNumber,"SR","","Documento sin respuesta","","","","");
									updateDocumentStatus(RUC, documentNumber,"SR","Sin respuesta SRI"); 
									processResponse(documentFileName,null,"SIN RESPUESTA| | |","N");
									
									LOGGER.debug("Envio correo error ");
									enviaEmailCliente("message_error", emite.getInfEmisor().getFecEmision(), "","","" ,"",(String) sriDocument.getExtraData().get("CorreoInterno"),documentNumber);
								} else if (infoAutorizacion[0].trim().contains("NO AUTORIZADO")){
									saveLog(documentNumber,"NA","","Documento no autorizado","","","","");
									updateDocumentStatus(RUC, documentNumber,"NA","No Autorizado por SRI");
									processResponse(documentFileName,null,respAutorizacion + "| | |","N");
									
									LOGGER.debug("Envio correo error ");
									enviaEmailCliente("message_error", emite.getInfEmisor().getFecEmision(), "","","" ,"",(String) sriDocument.getExtraData().get("CorreoInterno"),documentNumber);
								} else if (infoAutorizacion[0].equals("TRANSMITIDO SIN RESPUESTA")){
									
									sriDocument.getInfoTributaria().setTipoEmision("2");
									String ls_clave_contingencia = emite.obtieneClaveContingencia(emite.getInfEmisor().getRuc(), Integer.toString(emite.getInfEmisor().getAmbiente()), "0");
									//se genera nueva clave de contingencia
					        		String ls_clave_accesoCont = LeerDocumentos.generarClaveAccesoContingencia(emite,ls_clave_contingencia);
					        		//borro xml firmado 
					        		File delXML = new File(infoEmp.getDirFirmados() + documentFileName);
					        		if (delXML.exists()) {
					        			delXML.delete();
					        		}
					        		//hago backup de xml origen en recibido
					        		File xmlOrig = new File(infoEmp.getDirRecibidos()+documentFileName);
					        		FileUtils.copyFile(xmlOrig, new File(infoEmp.getDirRecibidos()+documentFileName + "_backup"));
					        		
					        		//modifico XML origen con nueva clave de acceso
					        		ModifyXML.modificarClaveAcceso(infoEmp.getDirRecibidos()+documentFileName, ls_clave_accesoCont);
					        		
					        		//se firma 
					        		resp = X509Utils.firmaValidaArchivo(documentFile , infoEmp.getDirFirmados(), RUC, Util.private_key_alias, infoEmp.getClaveFirma(), infoEmp.getRutaFirma());
					        		
					        		LOGGER.debug("Generando PreComprobante PDF " + infoEmp.getDirAutorizados()+FilenameUtils.removeExtension(documentFileName) + ".PDF");
									String pdfGenerated = generatePDF(sriDocument, "", "", true);
					        	
									enviaEmailCliente("message_error", emite.getInfEmisor().getFecEmision(), "","","",pdfGenerated,sriDocument.getExtraData().get("CorreoInterno"),documentNumber);
									
					        		processResponse(documentFileName,null,"TRANSMITIDO SIN RESPUESTA| | |","N");
									saveLog(documentNumber,"CT","","Contingencia por SRI","","","","");
									updateDocumentStatus(RUC, documentNumber,"CT","Contingencia por SRI."); 
								}
								
							}
		        	  } else if (respuestaRecepcion.getEstado().trim().equals("DEVUELTA")) {
		        		  LOGGER.debug("Documento devuelto  por SRI "+ documentNumber);
		        		  processResponse(documentFileName,respuestaRecepcion,"","S");
		        		  saveLog(documentNumber,"DS","","Devuelta por SRI","","","","");
		        		  updateDocumentStatus(RUC, documentNumber,"DS","Devuelta por SRI.");
		        		  delFile(emite,infoEmp.getDirFirmados(), infoEmp.getDirGenerado(), infoEmp.getDirNoAutorizados());
		        		  enviaEmailCliente("message_error", emite.getInfEmisor().getFecEmision(), "","","" ,"",(String) sriDocument.getExtraData().get("CorreoInterno"),documentNumber);
		        	  } else if (respuestaRecepcion.getEstado().trim().equals("SIN-RESPUESTA")) {
				...
				LOGGER.error("Error archivo " + documentFile.getName() + " : no creado o error en firmado ");
				processResponse(documentFileName,null,"Error archivo no creado o error en firmado| | |","N");
				enviaEmailCliente("message_error", emite.getInfEmisor().getFecEmision(), "","","" ,"",(String) sriDocument.getExtraData().get("CorreoInterno"),documentNumber);
				throw new Exception("Error archivo " + documentFile.getName() + " : no creado o error en firmado ");
			}
