...
public class ReaderDebitNote extends GenericReaderDocument {
	...
	public void readCSV(String file) {
		...
		try{
		    ...
		    while ((nextLine = reader.readNext()) != null) {
		    	...
		    	if(nextLine[0].equals("CD")){
		    		cabeceraInvoice(nextLine);
		    	}else if(nextLine[0].equals("DD")){
		    		detalleInvoice(nextLine);
		    		InvoiceDetailFieldList.add(InvoiceDetail);
		    	}
		    }
		
		    invoiceData.put("CD",InvoiceHeader);
		    invoiceData.put("DD",InvoiceDetailFieldList);
		     
		    invoiceStruct.put("08",invoiceData);
		    reader.close();
			}catch(Exception e){
				e.printStackTrace();
		  }			
	...
	private void cabeceraInvoice(String[] nextLineOut) {
    	...
    	InvoiceHeader.put("InvoiceTipoDocumento", nextLineOut[14].trim());
